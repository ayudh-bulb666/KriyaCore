...
### 3. First boot — creating your admin account

The tables are created during the build (`flask db upgrade` is part of
`buildCommand`), so they exist as soon as the first deploy goes green.

**On a paid plan** you can use Render's Shell tab and just run
`flask create-admin`.

**On the free plan there is no shell** — Render restricts SSH and the
dashboard shell to paid instances. That is not a blocker: Supabase is
reachable from anywhere, so run the command **from your own laptop against
the same database**. The account lands in Supabase either way; nothing about
it is specific to the machine that created it.

```bash
cd ~/Documents/Claude/GYMPro
export FLASK_APP=run.py
export DATABASE_URL='<the Supabase session pooler string from step 1>'
flask create-admin
```

Quote the connection string — it contains characters your shell would
otherwise interpret. It prompts for the email and a password (twice, hidden).

Two useful side effects of doing it this way: the same shell can run
`flask db upgrade` if you ever need to migrate by hand, and it proves your
`DATABASE_URL` is correct before you go looking at Render logs.
...
| | |
|---|---|
| **Sleeps after 15 min idle** | First visit after a quiet spell waits 30–60s |
| **No shell / SSH** | Run one-off commands locally against `DATABASE_URL`, as above |
| **No `preDeployCommand`** | Which is why migrations live in `buildCommand` |
| **750 instance-hours/month** | Enough for one always-on service, shared across all your free services |

All four disappear on Starter ($7/mo), which is a dropdown change in the
dashboard — no redeploy, no data loss.
